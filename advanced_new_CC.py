# -*- coding: utf-8 -*-
"""
Created on Fri Apr  4 16:33:00 2025
...
# Load the dataset
file_path = r'C:\Users\nfpm5\Downloads\Safa & Yusuf_updated_2\Electrode_data_new2.xlsx'
...
for rate in scan_rates:
    ...
    filtered_data = filtered_data[(filtered_data['Potential (V)'] >= 0) & (filtered_data['Potential (V)'] <= 0.4) &
                                  (filtered_data['Current (A)'] >= -0.06) & (filtered_data['Current (A)'] <= 0.06)]
...
for idx, rate in enumerate(scan_rates):
    ...
    ax.set_xlabel('Potential (V)', fontsize=18)
    ax.set_ylabel('Current (A)', fontsize=18)
    ax.set_title(f'{rate} mV/s Scan Rate', fontsize=18)
    ax.legend()
    ax.grid()
