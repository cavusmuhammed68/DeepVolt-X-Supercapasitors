...
for rate in scan_rates:
    plt.figure(figsize=(7, 6))  # Adjust figure size for clarity
...
for rate in scan_rates:
    ...
    scan_data[rate] = {
        'Potential (V)': potential[valid_data],
        'Current (A)': current[valid_data]
    }

# Plot DeepVolt-X data for scan rates 2, 5, 20, and 50 mV/s in the same figure
plt.figure(figsize=(10, 7))

for rate in scan_rates:
    plt.plot(scan_data[rate]['Potential (V)'], scan_data[rate]['Current (A)'], label=f'DeepVolt-X {rate} mV/s', linewidth=2)

plt.xlabel('Potential (V)', fontsize=14)
plt.ylabel('Current (A)', fontsize=14)
plt.title('DeepVolt-X Data for Scan Rates 2, 5, 20, 50 mV/s', fontsize=16)
plt.legend()
